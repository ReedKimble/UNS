...
3. **Continuity:** Smooth relational change is intrinsic to unity.  
   $\lim_{\Delta v_i \to 0} \bigl(\exists v_j\bigr)$
...
|------|------|-------------|
| `u` | `UValue` | A normalized value distribution (\Sigma = 1) |
...
**Expected Behavior:**  
When `state` is uniform, `field \approx 0`.  
...
Together they form the UNS analogs of **\nabla** and **\nabla×**, defining the relational dynamics of completeness.
