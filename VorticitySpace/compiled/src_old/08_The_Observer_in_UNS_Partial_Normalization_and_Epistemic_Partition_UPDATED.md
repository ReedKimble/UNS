...
|------|------|-------------|
| `u` | `UState` | The complete normalized system (\Sigma = 1). |
| `mask` | `UValue` | Boolean mask defining the observer subspace. |

### **Returns**
| Type | Description |
|------|-------------|
| `UState` | A reflexive substate normalized over its own subspace (\Sigma_S < 1). |

### **Operational Semantics**
1. Normalize full system `u`.  
2. Apply `mask` to isolate subspace $S$.  
3. Re-normalize subspace to local unity $\Sigma_S = 1_S < 1$.  
...
|------------------|-------------|--------------------|
| **helperNorm()** | Ensures total completeness (\Sigma = 1) | Defines global unity |
