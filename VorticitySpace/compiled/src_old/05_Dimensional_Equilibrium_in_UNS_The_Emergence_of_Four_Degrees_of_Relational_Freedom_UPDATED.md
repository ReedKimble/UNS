...
1. **Normalization:** Total completeness must remain invariant ($\Sigma = 1$).  
...
Dimensional equilibrium is indicated by minimal deviation ($\Delta \approx 0$) — a perfect self-referential fixed point.
...
The experiment is expected to show a near-zero deviation: `read(overlap | state(base))` $\approx 0$ at **D = 4**, confirming that 4D is the minimal complete relational topology.
...
$$
D_{min} = \min(D) : \exists W_0,\ \sum_i \lvert v_i \rvert^2 = 1,\ f(v_i) = v_i
$$
