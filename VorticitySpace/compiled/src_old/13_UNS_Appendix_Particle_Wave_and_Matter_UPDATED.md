...
It arises naturally from the interaction between **local normalization coherence (\Sigma_local)** and **global completeness (\Sigma_global = 1)**.

When a relational subset becomes locally normalized (\Sigma_local \approx 1), it manifests **particle-like stability**.  
When it remains distributed (\Sigma_local ≪ 1), it exhibits **wave-like propagation**.  
...
|-----------|------------|--------------|
| **Wave** | $ \partialρ/\partialx \approx 0, \partialΦ/\partialx ≠ 0 $ | Distributed phase continuity (delocalized normalization) |
| **Particle** | $ \partialρ/\partialx ≠ 0, \partialΦ/\partialx \approx 0 $ | Localized normalization attractor (stable relational density) |
...
$$
\Sigma_{local} : \Sigma_{distributed} \to \Sigma_{local}' : \Sigma_{distributed}'
$$
subject to $\Sigma_{\text{local}}' + \Sigma_{\text{distributed}}' = 1$.
...
$$
When $\mathcal{L}(v) \to 1$, the state behaves as a particle; when distributed, as a wave.
...
**Expected Results:**
- `density_local` increases \to particle-like coherence.  
- `density_global` remains invariant \to wave-like propagation.
...
|-------------|--------------------|
| **Particle** | Locally normalized subset (\Sigma_local \to 1) |
| **Wave** | Distributed phase field (\Sigma_local ≪ 1) |
| **Collapse** | Redistribution preserving \Sigma = 1 |
...
$$
These oppositely oriented attractors preserve \Sigma = 1 globally but exhibit opposite local curvature.  
